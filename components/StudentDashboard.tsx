...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ state, onUpdate }) => {
  ...
  const currentStudent = state.students.find(s => s.id === loggedStudentId) || state.students[0];
      ...
      {activeMainTab === 'beranda' ? (
        <>
          ...
          <div className="relative overflow-hidden bg-[#5b59e5] rounded-[3.5rem] p-12 text-white shadow-2xl shadow-indigo-200 flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="space-y-2 text-center md:text-left relative z-10">
              <h1 className="text-5xl font-black tracking-tight uppercase leading-tight">HALO, {currentStudent.name.split(' ')[0]}! 👋</h1>
              <p className="text-indigo-100 font-black text-[10px] uppercase tracking-[0.3em] opacity-80">KELAS: {currentStudent.class} • NIS: {currentStudent.nis}</p>
            </div>
            ...
            <button 
              ...
              className="relative z-10 bg-white text-[#5b59e5] px-10 py-5 rounded-[2rem] font-black text-[10px] uppercase tracking-widest flex items-center gap-3 hover:scale-105 transition-transform shadow-xl active:scale-95"
            >
              ...
              <section>
                <div className="flex justify-between items-center mb-6 px-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase">UJIAN AKTIF</h2>
                  </div>
                ...
                {activeExams.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {activeExams.map(exam => (
                      <div key={exam.id} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-100/50 flex flex-col justify-between hover:shadow-2xl transition-all group">
                         <div>
                            <div className="flex justify-between items-center mb-6">
                               ...
                               <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{exam.duration} MENIT</span>
                            </div>
                            <h3 className="text-xl font-black text-slate-800 uppercase mb-1 leading-tight">{exam.title}</h3>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{exam.subject}</p>
                         </div>
                         <button onClick={() => handleStartExam(exam)} className="w-full bg-[#1e293b] text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest mt-8 hover:bg-[#5b59e5] transition-all active:scale-95 shadow-lg shadow-slate-100">MULAI UJIAN</button>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="bg-white rounded-[3rem] border-2 border-dashed border-slate-100 py-20 flex flex-col items-center justify-center text-center shadow-inner">
                    ...
                    <p className="text-slate-300 font-black text-[10px] uppercase tracking-[0.2em]">BELUM ADA UJIAN AKTIF UNTUK KELAS ANDA.</p>
                  </div>
                )}
              </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <section>
                  <div className="flex items-center gap-3 mb-6 px-4">
                    ...
                    <h2 className="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase">HASIL TERAKHIR</h2>
                  </div>
                  <div className="bg-white rounded-[3rem] border border-slate-50 p-8 min-h-[250px] shadow-xl shadow-slate-100/50">
                    {studentResults.length > 0 ? (
                      <div className="space-y-4">
                         {studentResults.slice(0, 3).map((res, i) => (
                            <div key={i} className="flex justify-between items-center p-5 bg-slate-50 rounded-[2rem] border border-slate-100">
                               <div>
                                  <p className="font-black text-slate-800 text-[10px] uppercase tracking-tight">{state.exams.find(e => e.id === res.examId)?.title || 'Hasil Ujian'}</p>
                                  <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest mt-1">{new Date(res.timestamp).toLocaleDateString('id-ID')}</p>
                               </div>
                               <span className={`text-2xl font-black ${res.score >= 75 ? 'text-indigo-600' : 'text-red-500'}`}>{res.score.toFixed(0)}</span>
                            </div>
                         ))}
                      </div>
                    ) : (
                      <div className="h-full flex flex-col items-center justify-center py-10">
                        <p className="text-slate-200 font-black text-[10px] uppercase tracking-[0.2em] italic">BELUM ADA DATA.</p>
                      </div>
                    )}
                  </div>
                </section>
                ...
                <section>
                  <div className="flex items-center gap-3 mb-6 px-4">
                    ...
                    <h2 className="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase">RIWAYAT TUGAS</h2>
                  </div>
                  <div className="bg-white rounded-[3rem] border border-slate-50 p-8 min-h-[250px] shadow-xl shadow-slate-100/50">
                    {studentSubmissions.length > 0 ? (
                      <div className="space-y-4">
                         {studentSubmissions.slice(0, 3).map((sub, i) => (
                            <div key={i} className="flex justify-between items-center p-5 bg-slate-50 rounded-[2rem] border border-slate-100">
                               <div>
                                  <p className="font-black text-slate-800 text-[10px] uppercase tracking-tight">{sub.subject}</p>
                                  <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest mt-1">{new Date(sub.timestamp).toLocaleDateString('id-ID')}</p>
                               </div>
                               <span className={`text-[8px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest ${sub.status === 'GRADED' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>
            ...
            <div className="space-y-8">
               <section className="bg-white p-10 rounded-[3rem] shadow-2xl shadow-slate-200/40 border border-slate-50 h-full">
                  <div className="flex items-center gap-3 mb-10">
                    ...
                    <h2 className="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase">PAPAN PERINGKAT</h2>
                  </div>

                  <div className="bg-slate-50 p-1.5 rounded-[1.5rem] flex gap-1 mb-10 border border-slate-100 shadow-inner">
                    <button 
                      onClick={() => setLeaderboardTab('sekolah')}
                      className={`flex-1 py-3 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all ${leaderboardTab === 'sekolah' ? 'bg-white text-[#5b59e5] shadow-md border border-slate-100' : 'text-slate-400'}`}
                    >
                      GLOBAL
                    </button>
                    <button 
                      onClick={() => setLeaderboardTab('kelas')}
                      className={`flex-1 py-3 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all ${leaderboardTab === 'kelas' ? 'bg-white text-[#5b59e5] shadow-md border border-slate-100' : 'text-slate-400'}`}
                    >
                      KELAS
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                     {[...state.results]
                       .filter(r => leaderboardTab === 'sekolah' ? true : state.students.find(s => s.id === r.studentId)?.class === currentStudent.class)
                       .sort((a, b) => b.score - a.score)
                       .slice(0, 10)
                       .map((res, idx) => {
                          const student = state.students.find(s => s.id === res.studentId);
                          return (
                            <div key={idx} className={`flex items-center justify-between p-4 rounded-[1.8rem] transition-all border ${student?.id === currentStudent.id ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-transparent hover:border-slate-100'}`}>
                              <div className="flex items-center gap-4">
                                <div className={`w-8 h-8 flex items-center justify-center rounded-xl font-black text-[10px] ${idx === 0 ? 'bg-amber-400 text-white' : idx === 1 ? 'bg-slate-300 text-white' : idx === 2 ? 'bg-amber-700 text-white' : 'bg-slate-50 text-slate-300'}`}>
                                  {idx + 1}
                                </div>
                                <div>
                                   <p className="font-black text-slate-700 text-[10px] uppercase tracking-tight">{student?.name.split(' ')[0] || 'Siswa'}</p>
                                   <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest">{student?.class}</p>
                                </div>
                              </div>
                              <span className="font-black text-indigo-600 text-[11px]">{res.score.toFixed(0)}</span>
                            </div>
                          );
                       })}
                     {state.results.length === 0 && (
                       <p className="py-20 text-center text-slate-200 font-black text-[10px] uppercase tracking-widest italic">BELUM ADA DATA.</p>
                     )}
                  </div>
               </section>
            </div>
        ...
        <div className="bg-white p-12 rounded-[3.5rem] shadow-xl shadow-slate-100 border border-slate-50">
          <div className="flex justify-between items-center mb-12">
            <div>
              <h2 className="text-3xl font-black text-[#1e293b] uppercase tracking-tight">ANALISIS PERKEMBANGAN</h2>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300 p-4">
           <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
              <div className="px-10 py-8 border-b border-slate-50 flex justify-between items-center bg-white">
                 <h2 className="text-xl font-black text-[#1e293b] uppercase tracking-tight">KIRIM TUGAS MANDIRI</h2>
                 <button 
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="bg-[#f8fafc] border border-slate-100 rounded-[1.5rem] p-1.5 focus-within:ring-4 focus-within:ring-indigo-500/5 transition-all shadow-inner">
                       <input 
                         ...
                         placeholder="Mis: Fisika, Ekonomi..."
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="bg-[#f8fafc] border border-slate-100 rounded-[1.5rem] p-1.5 focus-within:ring-4 focus-within:ring-indigo-500/5 transition-all shadow-inner">
                       <textarea 
                         ...
                         placeholder="Ketik keterangan tugas Anda di sini..."
